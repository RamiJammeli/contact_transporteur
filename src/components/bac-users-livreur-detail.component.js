...
export default class BackUserLivreurDetail extends Component{
    ...
    constructor(props) {
      ...
      onSubmitSupprimer(e) {
        ...
        var dt_supp=supprimer==null? Date.now():null;
        if(supprimer==null){
          supprimer=dt_supp
          document.getElementById("activersupprimer").value="Activer"
          $("#spanSupprimer").text("(Compte supprimé depuis " +new Date().toLocaleDateString()+")");
        }else{
          supprimer=dt_supp
          document.getElementById("activersupprimer").value="Supprimer"
          $("#spanSupprimer").text("(Compte Activé)");
        }
        const user = {
          username: this.state.username,
          nom:this.state.nom,
    ...
    componentDidMount() {
       ...
       .then(response => {
        ...
        if(response.data.date_suppression!=null){
          supprimer=response.data.date_suppression;
          $("#spanSupprimer").text("(Compte supprimé depuis " +supprimer+")");
          document.getElementById("activersupprimer").value="Activer"
        }
       })
       .catch((error) => {
        ...
        <div className="portlet light bordered">
            <div className="portlet-title tabbable-line">
                <div className="caption caption-md">
                    ...
                    <span id="spanSupprimer"> (Compte Activé) </span>
                </div>
            </div>
